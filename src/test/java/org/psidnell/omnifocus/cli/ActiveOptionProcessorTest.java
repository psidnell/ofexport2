...
public class ActiveOptionProcessorTest {
    ...
    @Test
    public void testWeirdBugWhereQuotesGotEaten () throws Exception {

        // The library I'm using strips quotes from around arguments and thats really
        // inconvenient. I've fixed this in by subclassing BasicParser.
        Options options = new Options();
        options.addOption(new ActiveOption<ActiveOptionProcessorTest>("x", true, "i'm an opt", (p,o)->gotX(o), 0));

        ActiveOptionProcessor<ActiveOptionProcessorTest> p = new ActiveOptionProcessor<ActiveOptionProcessorTest>("test", options);
        p.processOptions(this, new String[]{"-x", "\"1\""}, 0);

        // Show that options processed in order
        assertEquals ("[X:\"1\"]", list.toString());
    }
